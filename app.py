...
def contar_stop_codons_como_terminadores(genes):
    """Cuenta STOP codons que actúan como terminadores (final de genes)"""
    stop_counts = {"TAA": 0, "TAG": 0, "TGA": 0}
    
    for gen in genes:
        # Tomar últimos 3 nucleótidos de cada gen (codon de terminación)
        if len(gen['secuencia']) >= 3:
            codon_terminal = str(gen['secuencia'][-3:]).upper()
            if codon_terminal in stop_counts:
                stop_counts[codon_terminal] += 1
    
    return stop_counts

def contar_stop_codons_totales(secuencia):
    """Cuenta TODOS los tripletes TAA, TAG, TGA en el genoma (no solo terminadores)"""
    secuencia = str(secuencia).upper()
    stop_counts = {"TAA": 0, "TAG": 0, "TGA": 0}
    
    # Buscar todos los tripletes (incluyendo en diferentes marcos de lectura)
    for i in range(len(secuencia) - 2):
        triplete = secuencia[i:i+3]
        if triplete in stop_counts:
            stop_counts[triplete] += 1
...
def index():
    ...
    # NUEVO: Contar STOP codons de DOS formas
    stop_codons_terminadores = contar_stop_codons_como_terminadores(genes)
    stop_codons_totales = contar_stop_codons_totales(secuencia_genoma)
    
    # Calcular proporciones para terminadores
    total_stop_term = sum(stop_codons_terminadores.values())
    proporciones_stop_term = {k: (v / total_stop_term) * 100 if total_stop_term > 0 else 0 for k, v in stop_codons_terminadores.items()}
    
    # Calcular proporciones para totales
    total_stop_tot = sum(stop_codons_totales.values())
    proporciones_stop_tot = {k: (v / total_stop_tot) * 100 if total_stop_tot > 0 else 0 for k, v in stop_codons_totales.items()}
    ...
    return render_template(
        ...
        top_genes=top_genes,
        stop_codons_terminadores=stop_codons_terminadores,
        stop_codons_totales=stop_codons_totales,
        stop_props_terminadores=proporciones_stop_term,
        stop_props_totales=proporciones_stop_tot,
        stop_codons=stop_codons_terminadores,   # ← NUEVA LÍNEA
        stop_props=proporciones_stop_term,      # ← la anterior
